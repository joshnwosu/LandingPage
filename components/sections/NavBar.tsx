...
export function NavBar({ items, className }: NavBarProps) {
        ...
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            if (matchingItem) {
              ...
              console.log(`Active section: ${matchingItem.name}`);
            }
          }
        });
      },
  ...
  return (
    <div
      className={cn(
        'fixed left-1/2 -translate-x-1/2 z-50 mb-6 pt-4 md:pt-6',
        className
      )}
    >
      <div className='flex items-center gap-3 bg-background/5 border border-border backdrop-blur-lg py-2 px-2 rounded-full shadow-lg'>
        {items.map((item) => {
          ...
          return (
            ...
            >
              <span className='hidden md:inline text-nowrap'>{item.name}</span>
